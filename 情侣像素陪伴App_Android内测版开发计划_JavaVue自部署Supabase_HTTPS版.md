...
| 具体使用时长展示 | 容易引发关系审计 |
| 地图定位 | 与产品核心无关且风险高 |
| 可装扮 / 可经营情侣房间系统 | 第二版留存功能；第一版仍保留首页动态状态房间，只不做装扮、经营、扩建 |
| 纪念日系统 | 非核心验证点 |
...
| status_code | 中文展示 | 候选房间区域 | 到达后动态表现 |
|---|---|---|---|
| `short_video` | 刷短视频中 | 沙发、床边、地毯 | 坐下 / 趴下刷手机，手机屏幕轻闪 |
| `watching_show` | 追剧中 | 电视前、沙发、床边 | 抱抱枕看电视，电视光轻闪 |
| `reading` | 阅读中 | 书桌、书架旁、地毯 | 拿书 / 电子书，偶尔翻页 |
| `music` | 听歌中 | 音响旁、窗边、地毯 | 戴耳机轻晃，音符冒出 |
| `gaming` | 游戏中 | 电视前、桌子旁、地毯 | 拿手柄 / 键盘，手柄或屏幕闪动 |
| `social` | 在线中 | 小桌旁、窗边、地毯 | 轻微待机，小气泡浮动，不显示聊天对象 |
| `online` | 在线 | 房间中央、窗边、门口 | 普通站立 / 呼吸，偶尔左右张望 |
| `resting` | 休息中 | 床上、懒人垫、窗边 | 睡觉 / 趴着休息，呼吸或月亮动画 |
| `offline` | 暂未更新 | 窗边、门口、房间角落 | 房间降饱和，灰色云朵或待机提示 |
| `paused` | 已暂停共享 | 门口、床边、房间角落 | 小锁 / 暂停符号，不展示具体活动 |

### 7.2 首页动态房间表现规则

首页不再把像素角色作为大头像展示，而是展示一个“TA 的小房间窗口”。

规则：

1. 房间是主体，像素小动物缩小到当前 Stitch 首页初稿约 1/3，让用户看到更多墙面、地面、家具和道具。
2. 每个状态绑定 2–3 个候选区域，候选区域来自同一个房间，例如沙发、床、桌子、电视、音响、书架、窗边、地毯、门口。
3. 收到当前 `status_code` 后，Vue 根据状态从候选区域中随机选择一个目标点。
4. 小动物先从当前位置走到目标区域。
5. 到达后再播放该状态对应动作，动作循环直到状态变化。
6. 状态变化后，小动物停止当前动作，再走向新状态的候选区域并切换动作。
7. 随机性只用于候选区域、路径、停留节奏和动作细节；状态对应的动作和道具必须准确，不能错配。

第一版开发要求覆盖所有 `7.1` 中列出的状态。D7 阶段只需要完成首页动态房间的大概设计稿和素材方向，不要求一次性完成所有状态的精细动画帧。

分工边界：

- Java 负责从包名、系统状态和后端数据得到抽象 `status_code`。
- Vue 只消费已经给定的 `status_code`，将其映射为视觉候选区域和动作表现。
- Vue 不维护具体 App 包名到状态的业务映射，也不参与状态识别、上传、后台任务或网络鉴权。
...
`allowed_statuses` 只表示用户可开关的活动类别。`offline` 和 `paused` 是系统状态，不参与类别开关，也不代表可被用户选择的活动状态。
...
- `properties` 不记录包名、具体 App 名称、使用时长、浏览内容、聊天对象。
- `properties` 不记录对方状态内容、对方 `status_code` 或可还原状态历史的时间序列。
- 与状态相关的埋点只记录布尔结果、错误码、耗时区间、是否成功等工程指标。
- `status_detected`、`status_uploaded`、`partner_status_viewed` 只能用于链路健康度统计，不能用于回放用户状态变化。
...
| 资产类型 | 数量 | 说明 |
|---|---:|---|
| 首页房间设计稿 | 1 套 | 一间可用于动态状态展示的像素房间，包含墙面、地面、床、沙发、桌子、电视、音响、书架、窗、地毯、门口等候选区域 |
| 基础角色 | 8 个 | 每个角色 1 张基础站立图，角色在首页房间中按小尺寸展示 |
| 角色移动帧 | 8 组 | 每个角色先准备 2–4 帧走路/待机帧，支持在房间候选区域之间移动 |
| 状态 / 互动道具 | 10 个 | 手机、电视、书、耳机、手柄、气泡、月亮、锁、云朵、爱心；`online` 使用无道具待机 |
| 全状态动作设计稿 | 10 类 | 覆盖 `short_video`、`watching_show`、`reading`、`music`、`gaming`、`social`、`online`、`resting`、`offline`、`paused`，D7 先做大概设计稿 |
...
app/src/main/assets/pixel/
  ...
  avatars/
    avatar_01_base.png
    avatar_01_walk_01.png
    avatar_01_walk_02.png
    avatar_02_base.png
    ...
    avatar_08_base.png
...
```text
Create a rough homepage room concept for a cute couple companion Android app.

Scene:
- cozy pixel-art room as the main visual area
- mobile app homepage composition
- the room should feel larger than the character
- include clear furniture zones: sofa, bed, rug, desk, TV, speaker, bookshelf, window, door
- leave enough open floor space for a small character to walk between zones
- no text inside the image
- soft pastel palette
- warm, low-pressure, private companion mood

Character scale:
- include one tiny placeholder pixel animal in the room
- character should be about one third of the size used in the current Stitch homepage draft
- character is not the center hero; the room is the center hero

State design notes:
- this is a rough design draft only
- all first-version states should be representable in this room:
  short video, watching show, reading, music, gaming, social, online, resting, offline, paused
- each state should have 2-3 possible furniture zones

Output:
- one homepage room mockup
- no text
- clean pixel grid
- suitable for later slicing into app assets
```
...
Character:
- small cute pixel animal
- compact body, simple face, rounded shape
- consistent animal silhouette across all frames
- no human clothing dependency unless the selected animal design needs a tiny accessory
...
6. social: idle with small abstract bubble, no chat content
7. online: idle standing and breathing
8. resting: sleeping with moon icon
9. paused: lock icon overlay
10. offline: gray cloud icon
11. heart interaction: small floating heart animation

Homepage room direction:
- the room is the main visual area
- the character is small, about one third of the current draft character size
- character can walk to furniture zones before playing the state action
- include clear candidate zones: sofa, bed, rug, desk, TV, speaker, bookshelf, window, door
- this stage only needs a rough design draft for all states, not final polished frames
...
1. 顶部：情侣状态、最近更新时间。
2. 中间：动态像素房间窗口，房间为主体，小动物在房间中移动和执行状态动作。
3. 次级：自己的共享状态卡片。
...
- 小动物尺寸约为当前 Stitch 首页初稿的 1/3，避免角色占据过多空间。
- 房间需要相对变大，能看清床、沙发、桌子、电视、音响、书架、窗边、地毯、门口等区域。
- 每个状态配置 2–3 个候选区域。
- 状态更新后，角色先走到该状态的候选区域，再播放对应动作。
- `short_video`、`watching_show`、`reading`、`music`、`gaming`、`social`、`online`、`resting`、`offline`、`paused` 都需要有对应的大概设计稿。
- 第一版实现可以先使用 CSS / 帧图 / 简单状态机实现粗粒度动画，不要求一开始完成精细角色寻路和完整逐帧动画。
...
| D7 | Codex/ImageGen 生成基础像素素材和首页动态房间大概设计稿，第一周整合测试 |
...
- Debug 版可打印最近前台 App 包名，仅限本机日志；Release 关闭，不上传、不展示给对方。
...
| D10 | 解绑逻辑和设置页入口 |
| D11 | 角色表、角色选择页、角色保存 |
| D12 | `status_map_cn.json` 和 Java 状态映射逻辑；Vue 只配置 `status_code` 到房间候选区域和动作表现 |
| D13 | `current_status` 上传，前台状态变化触发上传 |
| D14 | 获取对方状态，Vue 首页展示真实数据，并驱动动态房间状态动作 |
...
| D6 | 状态 PoC | UsageEvents 读取包名 | Debug 展示 | 无 | Debug 版可本机打印最近前台包名，Release 关闭 |
| D7 | 素材 | 资产加载器 | 首页动态房间大概设计稿 + 像素展示组件 | avatars seed | Vue 能展示房间和缩小后的像素角色 |
...
| D12 | 状态映射 | StatusMapper 将包名映射为 `status_code` | `status_code` 到房间区域 / 动作表现配置 | 无 | Java 可产出状态码，Vue 可按状态码展示候选区域和动作 |
| D13 | 上传状态 | StatusRepository 上传 | 自己状态卡 | current_status | A 状态写入服务端 |
| D14 | 对方状态 | partner status 拉取 | 首页动态房间真实展示 | 查询函数 | B 可看到 A 状态，角色先移动到对应区域再播放动作 |
...
| D21 | 整合 | 修主链路 bug | 错误态和全状态动态设计稿补齐 | 修函数 | 全链路可演示，首页状态表现不缺项 |
...
- [ ] 对方首页可以看到状态变化。
- [ ] 首页动态房间以房间为主体，像素角色缩小到 Stitch 初稿约 1/3。
- [ ] 首页角色收到状态后先移动到对应候选区域，再播放状态动作。
- [ ] 支持 `short_video`、`watching_show`、`reading`、`music`、`gaming`、`social`、`online`、`resting`、`offline`、`paused` 全部第一版状态的大概设计稿。
- [ ] 对方看不到具体 App 名称。
- [ ] 对方看不到使用时长。
- [ ] 不存在状态历史页面。
- [ ] 用户可以暂停共享。
...
| 素材来不及 | 中 | 全状态动态动作图不完整 | 先完成首页房间、候选区域、状态道具和全状态大概设计稿 | 8 角色静态图 + 统一房间 + 10 状态 icon，重点状态先做细化动画 | D7 |
...
埋点只用于判断关键链路是否跑通，不用于记录状态历史。任何埋点属性都不得包含包名、具体 App 名称、使用时长、浏览内容、聊天对象、对方状态内容、对方 `status_code`，也不得用连续时间序列还原用户状态变化。
...
| 可装扮 / 可经营情侣房间系统 | 作为第二版留存功能；不影响第一版首页动态状态房间 |
| 会员 / 装扮付费 | 验证后再做 |
